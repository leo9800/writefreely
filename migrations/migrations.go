...
func SetInitialMigrations(db *datastore) error {
	q := fmt.Sprintf("INSERT INTO appmigrations (version, migrated, result) VALUES (%s, %s, %s)", db.placeholder(1), db.now(), db.placeholder(2))
	log.Info(q)
	if _, err := db.Exec(q, "1", ""); err != nil {
		return err
	}
